...
class Dh_Handler(socketserver.BaseRequestHandler):
    ...
    def handle(self):
        ...
        elif self.state == 4 and bytearray(self.data) == b'3':
            port_range = self.request.recv(3072).strip().decode()  # 接收传递的端口范围字符串
            start_port, end_port = port_range.split('-')  # 根据分隔符拆分字符串为起始端口和结束端口
            start_port = int(start_port)
            end_port = int(end_port)

            open_ports = []
            for port in range(start_port, end_port + 1):
                result = subprocess.call(['nc', '-z', self.client_address[0], str(port)])
                if result == 0:
                    open_ports.append(port)

            response = ', '.join(str(port) for port in open_ports).encode()
            self.request.sendall(response)
